...
var renderComponent = (uuid, data, slotSettings) => {
  ...
  return /* @__PURE__ */ React.createElement(
    ...
    /* @__PURE__ */ React.createElement("div", { className: "w-full h-full rounded-md" }, rollPane)
  );
};
var dataValidator = (data) => {
  ...
  for (let sourceName in data) {
    let compatibleProperty = Object.keys(data[sourceName]).find((key) => {
      return key.includes("digitalDice") || key.includes("5eEncounter");
    });
    let property = void 0;
    ...
    for (let propertyName in data[sourceName]) {
      if (!propertyName.includes("digitalDice") && !propertyName.includes("5eEncounter"))
        continue;
...
var DiceRollDisplay_default = bundle;
export {
  DiceRollDisplay_default as default
};
